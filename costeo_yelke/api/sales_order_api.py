...
@frappe.whitelist()
def get_sales_order_form_defaults():
...
def save_sales_order(data):
    ...
    if name:
        doc = frappe.get_doc("Sales Order", name)
        if doc.docstatus != 0:
            frappe.throw("Solo se pueden editar órdenes de venta en borrador.")
    else:
        doc = frappe.new_doc("Sales Order")

    for field in [
        "customer", "company", "transaction_date", "delivery_date",
        ...
        "contact_email", "contact_mobile", "po_no",
    ]:
        val = data.get(field)
    ...
    frappe.db.commit()
    return get_sales_order(doc.name)
...
def submit_sales_order(name):
    doc = frappe.get_doc("Sales Order", name)
    if doc.docstatus != 0:
        frappe.throw("La orden de venta ya fue enviada o cancelada.")
    doc.flags.ignore_permissions = True
    doc.flags.ignore_mandatory   = True
    doc.flags.ignore_links       = True
    doc.submit()
...
def make_sales_order_from_quotation(quotation_name):
    """Create a Sales Order from a submitted Quotation, setting delivery_date automatically."""
    from erpnext.selling.doctype.quotation.quotation import make_sales_order as _make_so

    so = _make_so(quotation_name)
    ...
    so.insert()
    frappe.db.commit()
    return {"name": so.name, "doctype": "Sales Order"}


@frappe.whitelist()
def get_item_details_for_so(item_code, price_list=None):
