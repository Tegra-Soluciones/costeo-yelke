<template>
  <div class="flex flex-col min-h-0">
    ...
    <!-- Header -->
    <PageHeader
      :title="isNew ? 'Nueva Cotización' : (quot.name || 'Cargando…')"
      :subtitle="isNew ? (prefillCosteo ? `Desde Costeo: ${prefillCosteo}` : 'Nueva cotización') : (quot.customer_name || quot.party_name || '')"
      back
    >
      <template v-if="!isNew">
        ...
        <!-- Acciones solo para cotizaciones validadas -->
        <template v-if="quot.docstatus === 1">
          <!-- Botón principal: Enviar (abre modal de medios) -->
          <button
          ...
          <button
            :disabled="acting === 'so'"
            class="px-3 py-1.5 border border-gray-200 text-gray-600 text-sm font-medium rounded-md hover:bg-gray-50 flex items-center gap-1.5 transition-colors disabled:opacity-50"
            @click="makeSO"
          >
        ...
        <div class="w-px h-5 bg-gray-200" />
        <a
          :href="`/app/quotation/${encodeURIComponent(quot.name)}`"
          target="_blank"
    ...
    <div v-else class="p-5 pb-40 flex gap-5 items-start">
      ...
      <div class="flex-1 min-w-0 space-y-4">
        ...
        <div v-if="isDirty && canEdit" class="flex items-center justify-between bg-amber-50 border border-amber-200 rounded-xl px-4 py-2.5">
          ...
          <div class="flex items-center gap-2">
            <button v-if="!isNew" class="px-3 py-1.5 border border-gray-200 text-gray-500 text-xs font-medium rounded-lg hover:bg-gray-50" @click="discardChanges">
              Descartar
            </button>
        ...
        <div class="bg-white rounded-b-xl rounded-tr-xl border border-gray-200 p-6">
          ...
          <div v-if="activeTab === 'general'" class="space-y-5">
            <div class="grid grid-cols-2 gap-x-6 gap-y-4">
              ...
              <div>
                <label class="field-label">Cotización para</label>
                <select v-model="form.quotation_to" :disabled="!canEdit" class="field-input" @change="form.party_name = ''">
              ...
              <div>
                ...
                <input v-model="form.valid_till" type="date" :disabled="!canEdit" class="field-input" />
              </div>
          ...
          <div v-if="activeTab === 'items'" class="space-y-4">
              ...
              <table class="w-full text-sm min-w-[760px]">
                ...
                <tbody class="divide-y divide-gray-50">
                  <tr
                    ...
                    :key="idx"
                    class="group"
                    :class="row._dirty ? 'bg-amber-50/20' : ''"
                  >
                    ...
                    <td v-if="canEdit" class="py-3">
                      <button
                        class="w-6 h-6 flex items-center justify-center rounded opacity-0 group-hover:opacity-100 hover:bg-red-50 hover:text-red-500 text-gray-300 transition-all"
                  ...
                  <tr v-if="!form.items.length">
                    <td :colspan="canEdit ? 8 : 7" class="py-10 text-center text-gray-400 text-sm">
                      Sin artículos. {{ canEdit ? 'Agrega artículos abajo.' : '' }}
                    </td>
                  </tr>
                </tbody>
              </table>
            ...
            <button
              v-if="canEdit"
              class="flex items-center gap-2 px-4 py-2 border border-dashed border-gray-300 rounded-xl text-sm text-gray-500 hover:border-brand-400 hover:text-brand-600 transition-colors w-full justify-center"
              @click="addItem"
            >
            ...
            <div v-if="canEdit || form.additional_discount_percentage" class="flex items-center gap-4 pt-3 border-t border-gray-100">
              ...
              <input
                ...
                :disabled="!canEdit"
                class="field-input w-24 text-right text-xs"
              />
            </div>
          </div>
          ...
          <div v-if="activeTab === 'terminos'" class="space-y-4">
            ...
            <div>
              ...
              <textarea v-model="form.terms" :disabled="!canEdit" rows="8" class="field-input resize-y" placeholder="Términos y condiciones de la cotización…" />
            </div>
          </div>
        ...
        <div v-if="canEdit" class="flex items-center justify-between gap-3">
          ...
          <div class="flex items-center gap-3">
            <!-- Guardar borrador (only when already saved, to avoid confusion on new docs) -->
            <button
              v-if="!isNew"
            ...
            <!-- Validar: guarda si hay cambios y luego hace submit (docstatus 0→1) -->
            <button
              :disabled="!!(saving || acting)"
            ...
            >
              ...
              {{ saving ? 'Guardando…' : acting === 'submit' ? 'Validando…' : 'Validar' }}
            </button>
          </div>
        </div>

        <div v-if="!isNew && quot.docstatus === 1" class="flex justify-end">
          <button
      ...
      <div class="w-60 flex-shrink-0 space-y-4 sticky top-5 self-start max-h-[calc(100vh-80px)] overflow-y-auto">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-4 space-y-2.5">
          ...
          <div v-if="quot.total_taxes_and_charges" class="flex justify-between items-center text-sm">
            <span class="text-gray-500">Impuestos</span>
            <span class="font-medium text-gray-800">{{ fmtC(quot.total_taxes_and_charges) }}</span>
          </div>
        ...
        <!-- Contact card -->
        <div v-if="form.party_name" class="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
          ...
          <div v-if="form.contact_mobile" class="flex items-center gap-2">
            <svg class="w-3.5 h-3.5 text-green-500 flex-shrink-0" viewBox="0 0 24 24" fill="currentColor">
            ...
            <button
              v-if="!isNew"
              class="text-xs text-green-600 hover:text-green-700 font-medium truncate"
              @click="openWhatsApp"
            >{{ form.contact_mobile }}</button>
            <span v-else class="text-xs text-gray-600 truncate">{{ form.contact_mobile }}</span>
          </div>
          <RouterLink
            :to="`/cotizaciones?customer=${encodeURIComponent(form.party_name)}`"
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-4 space-y-2.5 text-sm">
          <div v-if="!isNew">
            ...
            <StatusBadge :docstatus="quot.docstatus" :status="quot.status" class="mt-0.5" />
          </div>
          <div v-if="form.transaction_date">
            <p class="text-xs text-gray-400">Fecha</p>
            <p class="font-medium text-gray-700">{{ fmtDate(form.transaction_date) }}</p>
          </div>
          <div v-if="form.valid_till">
            <p class="text-xs text-gray-400">Válida hasta</p>
            <p class="font-medium" :class="isExpired(form.valid_till) ? 'text-red-500' : 'text-gray-700'">
              {{ fmtDate(form.valid_till) }}
            </p>
          </div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-4">
          ...
          <button
            v-if="canEdit"
            class="mt-2 w-full text-xs text-brand-600 hover:text-brand-700 font-medium text-center py-1"
        ...
        <div v-if="!isNew" class="bg-white rounded-xl border border-gray-200 p-4 space-y-2 text-xs text-gray-400">
          <div v-if="quot.creation">
            <p class="font-medium text-gray-500">Creada</p>
            <p>{{ fmtDatetime(quot.creation) }}</p>
          </div>
          <div v-if="quot.modified">
            <p class="font-medium text-gray-500">Modificada</p>
            <p>{{ fmtDatetime(quot.modified) }}</p>
          </div>
        </div>
        ...
        <div v-if="showEmailModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm" @click.self="showEmailModal = false">
          <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg">
            ...
            <div class="px-6 py-4 space-y-4">
              <div>
                <label class="field-label">Para <span class="text-red-400">*</span></label>
                <input v-model="emailForm.to" type="email" class="field-input" placeholder="correo@cliente.com" />
              </div>
              <div>
                <label class="field-label">CC</label>
                <input v-model="emailForm.cc" type="text" class="field-input" placeholder="otro@ejemplo.com" />
              </div>
              <div>
                <label class="field-label">Asunto</label>
                <input v-model="emailForm.subject" type="text" class="field-input" />
              </div>
              <div>
                <label class="field-label">Mensaje</label>
                <textarea v-model="emailForm.message" rows="5" class="field-input resize-y" />
              </div>
              <div>
                <label class="field-label">Formato de impresión</label>
                <select v-model="emailForm.print_format" class="field-input">
              ...
              <label class="flex items-center gap-2 cursor-pointer">
                ...
                <span class="text-sm text-gray-700">Adjuntar PDF de la cotización</span>
              </label>
            </div>
      ...
      <Transition name="modal">
        <div v-if="showWAModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm" @click.self="showWAModal = false">
          <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            ...
            <div class="px-6 py-4 space-y-4">
              <div>
                <label class="field-label">Número de WhatsApp <span class="text-red-400">*</span></label>
                <input v-model="waPhone" type="tel" class="field-input" placeholder="+52 55 1234 5678" />
                <p class="text-xs text-gray-400 mt-1">Incluye código de país, ej: +521234567890</p>
              </div>
              <div>
                <label class="field-label">Formato PDF</label>
                <select v-model="waPrintFormat" class="field-input">
                  <option v-for="pf in printFormats" :key="pf.name" :value="pf.name">{{ pf.name }}</option>
                </select>
              </div>
              <div>
                <label class="field-label">Mensaje</label>
                <textarea v-model="waMessage" rows="4" class="field-input resize-y text-sm" />
              </div>
              <div class="bg-amber-50 border border-amber-200 rounded-lg px-4 py-3 text-xs text-amber-700">
                <p class="font-semibold mb-1">📎 Cómo adjuntar el PDF:</p>
                <ol class="list-decimal ml-4 space-y-0.5">
                  <li>Haz clic en <strong>Enviar</strong> — se descargará el PDF y se abrirá WhatsApp Web</li>
                  <li>En WhatsApp Web, usa el botón de adjuntar (📎) y selecciona el PDF descargado</li>
                </ol>
              </div>
            </div>
            <div class="flex justify-end gap-3 px-6 py-4 border-t border-gray-100">
              ...
              <button
                ...
                class="px-5 py-2 bg-green-500 text-white text-sm font-semibold rounded-xl hover:bg-green-600 disabled:opacity-50 flex items-center gap-2"
                @click="sendWhatsApp"
              >
      ...
      <Transition name="modal">
        <div v-if="showSendModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm" @click.self="showSendModal = false">
          <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <h3 class="text-base font-semibold text-gray-800">Enviar cotización</h3>
            ...
            <div class="px-6 py-5 space-y-3">
              <!-- Email -->
              <button
                class="w-full flex items-center gap-4 px-4 py-3.5 border border-gray-200 rounded-xl hover:border-brand-400 hover:bg-brand-50 transition-colors text-left group"
                @click="showSendModal = false; showEmailModal = true"
              >
                ...
                <div>
                  ...
                  <p class="text-xs text-gray-400 mt-0.5">Enviar PDF adjunto por email</p>
                </div>
              </button>

              <!-- WhatsApp -->
              <button
                class="w-full flex items-center gap-4 px-4 py-3.5 border border-gray-200 rounded-xl hover:border-green-400 hover:bg-green-50 transition-colors text-left group"
                @click="showSendModal = false; openWhatsApp()"
              >
                ...
                <div>
                  ...
                  <p class="text-xs text-gray-400 mt-0.5">Abrir WhatsApp Web con mensaje</p>
                </div>
              </button>

              <!-- Imprimir -->
              <button
                class="w-full flex items-center gap-4 px-4 py-3.5 border border-gray-200 rounded-xl hover:border-gray-400 hover:bg-gray-50 transition-colors text-left group"
                @click="showSendModal = false; openPrint()"
              >
                ...
                <div>
                  <p class="text-sm font-semibold text-gray-800">Imprimir / PDF</p>
                  <p class="text-xs text-gray-400 mt-0.5">Ver vista previa de impresión</p>
                </div>
              </button>
            </div>
          </div>
...
const StatusBadge = defineComponent({
  ...
  setup(p) {
    const LABELS  = { Open: "Abierta", Ordered: "Ordenada", Lost: "Perdida", Draft: "Borrador", Cancelled: "Cancelada" };
    const COLORS  = {
      0: "bg-gray-100 text-gray-600",
      1: { Open: "bg-blue-100 text-blue-700", Ordered: "bg-green-100 text-green-700", Lost: "bg-red-100 text-red-600" },
    ...
    return () => {
      ...
      const cls   = ds === 0 ? COLORS[0] : ds === 2 ? COLORS[2] : (COLORS[1][p.status] || "bg-blue-100 text-blue-700");
...
// WhatsApp state
const waPhone       = ref("");
const waPrintFormat = ref("Standard");
const waMessage     = ref("");

// Email form
const emailForm = reactive({
  to: "", cc: "", subject: "", message: "", print_format: "Standard", attach_pdf: true,
});

const isNew         = computed(() => !props.name || props.name === "nueva");
const prefillCosteo = route.query.costeo || "";
...
// ── Computed ──────────────────────────────────────────────────────────────────
const canEdit = computed(() => isNew.value || quot.docstatus === 0);
...
const totals = computed(() => {
  ...
  const taxes = isNew.value ? 0 : (quot.total_taxes_and_charges || 0);
...
async function fillItemDefaults(row) {
  ...
  row._dirty = true;
  try {
    const res = await call("costeo_yelke.api.quotation_api.get_item_details_for_quotation", {
...
// ── Sync form ─────────────────────────────────────────────────────────────────
function syncFromQuotation(data) {
  Object.assign(quot, {
...
function discardChanges() {
  if (formOriginal.value && Object.keys(formOriginal.value).length) {
    ...
    Object.keys(form).forEach(k => {
      if (k !== "items") form[k] = saved[k] ?? form[k];
    });
...
async function save() {
  ...
  try {
    ...
    const saved = await call("costeo_yelke.api.quotation_api.save_quotation", {
      data: JSON.stringify(payload),
    });
    syncFromQuotation(saved);
    showToast(isNew.value ? "Cotización creada" : "Cotización guardada");
    if (isNew.value) {
      router.replace(`/cotizaciones/${encodeURIComponent(saved.name)}`);
    }
    ok = true;
  } catch (e) {
...
async function cancel() {
  ...
  acting.value = "cancel";
  try {
    const res = await call("costeo_yelke.api.quotation_api.cancel_quotation", { name: quot.name });
    Object.assign(quot, { docstatus: res.docstatus, status: res.status });
...
async function makeSO() {
  acting.value = "so";
  try {
    const res = await call("costeo_yelke.api.quotation_api.make_sales_order", { name: quot.name });
    showToast(`Orden de Venta creada: ${res.name}`);
    setTimeout(() => router.push(`/ordenes-venta/${encodeURIComponent(res.name)}`), 1000);
  } catch (e) {
    showToast(e.message || "Error al crear Orden de Venta", "error");
  } finally {
    acting.value = "";
  }
}
...
async function openPrint() {
  const fmt = await defaultPrintFormat("Quotation");
  window.open(
    `/printview?doctype=Quotation&name=${encodeURIComponent(quot.name)}&format=${encodeURIComponent(fmt)}&no_letterhead=0`,
    "_blank"
  );
}
...
function prepareEmailForm() {
  ...
  emailForm.cc      = "";
  emailForm.subject = `Cotización ${quot.name} - ${customerDisplayName.value}`;
  emailForm.message = `Estimado cliente,\n\nAdjunto encontrará la cotización ${quot.name} por un total de ${fmtC(totals.value?.grand || 0)} ${form.currency}.\n\nQuedo a sus órdenes para cualquier consulta.\n\nSaludos`;
  emailForm.print_format = printFormats.value[0]?.name || "Standard";
...
async function sendEmail() {
  ...
  try {
    ...
    await call("costeo_yelke.api.quotation_api.send_quotation_email", {
      ...
      print_format: emailForm.print_format,
      attach_pdf:   emailForm.attach_pdf ? 1 : 0,
    });
    showToast("Correo enviado correctamente");
    showEmailModal.value = false;
  } catch (e) {
    showToast(e.message || "Error al enviar correo", "error");
  } finally {
...
function openWhatsApp() {
  ...
  waPrintFormat.value = printFormats.value[0]?.name || "Standard";
  waMessage.value     = `Hola, le comparto la cotización *${quot.name}* por un total de *${fmtC(totals.value?.grand || 0)} ${form.currency}*.\n\nAdjunto el PDF con el detalle. Quedo a sus órdenes.`;
...
function sendWhatsApp() {
  ...
  const pdfUrl = `/api/method/frappe.utils.print_format.download_pdf?doctype=Quotation&name=${encodeURIComponent(quot.name)}&format=${encodeURIComponent(waPrintFormat.value)}&no_letterhead=0`;
  const a = document.createElement("a");
  a.href = pdfUrl;
  a.download = `${quot.name}.pdf`;
  a.click();

  // Open WhatsApp Web after a short delay
  setTimeout(() => {
    const waUrl = `https://wa.me/${phone}?text=${encodeURIComponent(waMessage.value)}`;
...
onMounted(async () => {
  loading.value = true;
  try {
    const [defs, fmts] = await Promise.all([
      call("costeo_yelke.api.quotation_api.get_quotation_form_defaults"),
      call("costeo_yelke.api.quotation_api.get_quotation_print_formats"),
      loadItemIndex(),
    ]);
    ...
    if (fmts?.length) {
      printFormats.value = fmts;
      emailForm.print_format = fmts[0].name;
      waPrintFormat.value    = fmts[0].name;
    }
    ...
    if (!isNew.value) {
      const data = await call("costeo_yelke.api.quotation_api.get_quotation", { name: props.name });
      syncFromQuotation(data);
      ...
      if (pre) {
        ...
        form.transaction_date  = pre.transaction_date  || today();
        form.taxes_and_charges = pre.taxes_and_charges || "";
        form.items = (pre.items || []).map(r => ({
          ...r, image: "", amount: (r.rate || 0) * (r.qty || 1), _dirty: true,
        }));
      }
      formOriginal.value = {};
    }
...
// Watch email modal open to refresh defaults
watch(showEmailModal, (v) => { if (v) prepareEmailForm(); });

watch(() => form.items, (items) => {
  items.forEach(r => recalcRow(r));
}, { deep: true });
</script>
