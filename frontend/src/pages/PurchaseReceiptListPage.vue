...
function openRow(pr) {
  if (pr.costeo) router.push({ name: "CosteoDetail", params: { name: pr.costeo }, query: { step: "4", highlight: pr.name, doctype: "Purchase Receipt" } });
}

onMounted(async () => {
  try { rows.value = await call("costeo_yelke.api.documentos_api.get_purchase_receipts", { limit: 100 }) || []; }
